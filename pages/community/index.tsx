	...
	return (
		<div className="py-16 px-4 space-y-8">
			{[...new Array(10)].map((_, idx) => (
				<div key={idx} className="flex flex-col items-start cursor-pointer">
					<span className="flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800">
						동네질문
					</span>
					<div className="mt-2 text-gray-700" onClick={() => router.push(`/community/${idx}`)}>
			...
			<button className="fixed shadow-xl bottom-20 right-5 bg-orange-400 rounded-full p-4 text-white hover:bg-orange-500 cursor-pointer">
				<svg
					className="w-6 h-6"
					fill="none"
					stroke="currentColor"
