...
interface UploadItemsForm {
  name: string;
  price: string;
  description: string;
}
interface UploadItemsMutation {
  ok: boolean;
  product: Product;
}
...
const Upload: NextPage = () => {
  ...
  const { register, handleSubmit } = useForm<UploadItemsForm>({});
  const [uploadItems, { loading, data }] = useMutation<UploadItemsMutation>("/api/products");

  const onValid = (data: UploadItemsForm) => {
    if (loading) return;
