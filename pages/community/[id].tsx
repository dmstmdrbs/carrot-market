...
const CommunityPostDetail: NextPage = () => {
  ...
  const { data, error } = useSWR<PostResponse>(router.query.id ? `/api/posts/${router.query.id}` : null);
  useEffect(() => {
    if (data && data?.ok && data.post === null) {
      router.push("/404");
    }
  }, [data]);
  return (
      ...
      <div>
        ...
        <div className="flex mb-3 px-4 cursor-pointer pb-3  border-b items-center space-x-3">
          ...
          <div>
            ...
            <Link href={`/users/profiles/${data?.post?.user?.id}`} passHref>
